...
public class UserController {
	...
	public Result<Map<String, Object>> login(User user, Integer vercode, HttpServletRequest request) {
		...
		Integer sessionVercode = (Integer) redisUtil.get(GlobalConst.VALI_SESSION_KEY.getKey() + ":" + session.getId());
		if (sessionVercode != null && (vercode.intValue() == sessionVercode.intValue())) {
			...
			if (userResult != null && !userResult.equals("")) {
				...
				return userTokenFlag && userPermissionsFlag && userInfoFlag ? ResultUtil.success(200, "登录成功！", mapToken)
						: ResultUtil.error(500, "登录失败！");
			} else {
	...
	public void number(HttpServletResponse response, HttpSession httpSession) throws IOException {
		...
		redisUtil.set(GlobalConst.VALI_SESSION_KEY.getKey() + ":" + httpSession.getId(), count,300);//验证码300秒刷新
		// 生成图片验证码
